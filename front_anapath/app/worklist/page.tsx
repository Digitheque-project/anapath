...
export default function WorklistPage() {
  ...
  const filteredRequests = requests.filter(req => {
    ...
    if (filter === 'pending') return req.statut === 'CREEE' || req.statut === 'EN_ATTENTE';
    if (filter === 'validated') return req.statut === 'VALIDE';
    return true;
  });
  ...
  const getStatusBadge = (statut: string) => {
    switch (statut) {
      case 'VALIDE':
        return <span className="px-2 py-0.5 rounded bg-green-100 text-green-700 text-[10px] font-bold">✓ Validé</span>;
      case 'RESULTAT_DISPONIBLE':
        return <span className="px-2 py-0.5 rounded bg-yellow-100 text-yellow-700 text-[10px] font-bold">📋 Résultat dispo</span>;
      case 'CREEE':
      case 'EN_ATTENTE':
        return <span className="px-2 py-0.5 rounded bg-orange-100 text-orange-700 text-[10px] font-bold">⏳ En attente</span>;
      default:
        return <span className="px-2 py-0.5 rounded bg-gray-100 text-gray-600 text-[10px] font-bold">{statut}</span>;
    }
  };
      ...
      <main className="flex-1 ml-64 min-h-screen flex flex-col w-[calc(100%-256px)]">
        ...
        <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl flex justify-between items-center px-6 py-3 shadow-sm">
          <div className="flex items-center gap-4 flex-1">
            <h2 className="text-lg font-black text-blue-900 tracking-tight">Fil de travail</h2>
            <div className="flex items-center bg-[#ecedf6]/50 px-3 py-1.5 rounded-full max-w-md w-full">
              ...
              <input className="bg-transparent border-none focus:ring-0 text-sm w-full" placeholder="Rechercher..." type="text"/>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button className="p-2 hover:bg-slate-100 rounded-full"><span className="material-symbols-outlined">notifications</span></button>
            <button className="p-2 hover:bg-slate-100 rounded-full"><span className="material-symbols-outlined">settings</span></button>
            <div className="w-8 h-8 rounded-full bg-[#00478d]/10 flex items-center justify-center"><span className="material-symbols-outlined text-[#00478d] text-sm">person</span></div>
          </div>
        </header>
        ...
        <div className="flex-1 p-6 w-full">
              ...
              <table className="w-full text-sm">
                <thead className="bg-[#f2f3fb] text-[11px] font-bold text-slate-500 uppercase">
                  <tr>
                    ...
                    <th className="p-4 text-left">Prélèvement</th>
                ...
                <tbody className="divide-y divide-outline-variant/10">
                  {filteredRequests.map((req) => (
                    <tr key={req.id} className="hover:bg-slate-50/80 transition-colors group">
                      ...
                      <td className="p-4">
                        <span className="px-2 py-0.5 rounded bg-blue-50 text-blue-700 text-[10px] font-bold">{req.typeExamen}</span>
                      </td>
          ...
          <div className="mt-6 text-center text-xs text-slate-400">
            Total: {filteredRequests.length} demande(s)
          </div>
        </div>
      </main>
    </div>
